...
- The backend REST envelope is `{ success, message, data }` with paginated lists as `{ count, next, previous, results }`; fetch it client-side through `lib/http` plus TanStack Query.
