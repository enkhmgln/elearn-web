...
- Auth routes use `app/(auth)/`; public marketing/legal use `app/(public)/`. Root metadata lives in `lib/metadata.ts`: default title is `constants.APP_NAME`, other pages use the template `AppName | %s`. `app/not-found.tsx` is inline (no separate NotFoundView).
